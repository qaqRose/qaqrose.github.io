...
分别是 [ShiningPanda]([ShiningPanda | Jenkins plugin](https://plugins.jenkins.io/shiningpanda/))  和 [pyenv-pipeline]([plugins.jenkins.io](https://plugins.jenkins.io/pyenv-pipeline))
...
Pyenv Pipeline则是流水线（Pipeline）专用，提供了`withPythonEnv`语法块，是现代流水线的首先方案，还可以复用ShiningPanda注册的全局Python名称
...
这里需要注意的是，Python的路径直接覆盖到python执行文件，而不是目录（在python执行文件的同目录下的pip执行可以被识别到执行）
...
这样就把镜像推送到生产的harbor了，剩下就生产部署的问题，不在这里过多赘述
